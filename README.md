...
Презентация: [тык](https://drive.google.com/file/d/1YOHltB6nh-ZIAkd-51Shy1KuCuybsGTR/view?usp=sharing)
...
- Для всего датасета рассчитываются:
  ...
  - Общий штраф `feee_in_usdt`, исходя из лимитов, которые были закрыты или не закрыты. Это штраф составляет 1% от первого лимита `LIMIT_MIN` для каждого терминала, если суммарная сумма у терминала меньше `LIMIT_MIN`.
...
## Наши преимущества:
- создали свой симулятор конвейера и проводим оптимизацию по нему
